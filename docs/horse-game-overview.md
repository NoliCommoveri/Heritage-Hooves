...
**Built 3 Aug 2026 (amendment 0017a §6):** admission to play is now an explicit operator control, `breeds.enabled`, toggled from `/admin/breeds`. Disabling a breed closes it to *new arrivals only* — founding offers, the consignment dealer, admin-created horses, new NPC breeding targets, new show classes — and never touches a horse, pedigree, class or listing that already exists; two existing horses of a disabled breed can still be bred and their foal is still that breed.

**Update 4 Aug 2026:** all eight breeds' ideal vectors are now seeded (`migrations/0107`) and every breed-conformation show class exists for all eight, with the NPC show barn stocking each one to its own target (`docs/breed-ideal-vectors.md` §6, that date's build-log entry). **Second update, same day:** the consignment dealer followed — it now mints from whatever breeds are in play (`breeds.enabled`) directly, rather than a second `consignment_breed_codes` allowlist that had drifted out of sync and kept it Quarter-Horse-only after the vectors above widened everything else. That config key is gone (migration `0109`); `breeds.enabled` from `/admin/breeds` is now the one control for which breeds the game admits anywhere.
...
**Selling, both buying routes, and stud services are now built (2026-08-03/2026-08-04)** — `docs/slices/0017-market.md` Parts A-D. Sale listings are Part A's `appraise()` (§14 of that document has the exact formula, close to but not identical to this section's list — it adds a colour term via amendment 0017a and deliberately never reads pedigree, an open question left for a future session, §18). Buy offers are Part C's standing-offers board plus an NPC stable shopping the open market on its own tick cycle, both reusing the exact appraisal, target-scoring and sale path the rest of the market already uses — no second "how good is this horse" formula and no second sale batch.

**Stud services (Part D, 2026-08-04)** built the first cross-stable breeding in the game: `stud_listings` and `stud_bookings` (§13 of that document), a `/market/stud` browse-and-book flow, and NPC stallions offered automatically alongside player ones. The operator settled Part D's three open questions when commissioning it: **no live-foal guarantee** (a stud fee pays for the covering, not the outcome), **the market's commission applies to a stud fee exactly as it applies to a sale**, and **NPC stallions stand at stud too** — the whole point this section names ("a cheaper, more realistic, better-targeted outcross mechanism than outright sales"). The one new mechanism Part D adds beyond the sale path: an NPC stallion is withdrawn from stud the moment his realised quality crosses the NPC ceiling (§10d below), because breeding to one *combines* his genetics with a player's own stock — unlike buying an NPC horse outright, which only ever hands over that horse's own ceiling-bound quality, so the ceiling has to be enforced again here rather than assumed to still hold.
...
- **§10f** — **NPC selling, both buying routes, and stud services are now built** (`docs/slices/0017-market.md` Parts B, C and D, 2026-08-03/2026-08-04) — see this section's own update above. Stud does not change an NPC stable's own population at all (§13.3 of that document: the foal is born into the *mare's* owner's barn, under their prefix) — it is purely an outcross route for a player's own line, the "cheaper, more realistic, better-targeted" mechanism this section names. An NPC stable's own population is still bounded by `capacity` with no culling mechanism, exactly as §10d describes; the operator's manual outcross batch on `/admin/npc` (§3.3 of that slice) remains the only way *new* genetic material enters an NPC stable's own closed line.
...
**Market** — NPC sale listings, buy offers, stud services, player-to-player trading. **All four built** — player-to-player trading and NPC sale listings/buy offers as of 3 Aug 2026, stud services (Parts A-D of `docs/slices/0017-market.md`) as of 4 Aug 2026. The minimum listing duration this line used to promise was considered and declined — see §1a.

**Training** (§8c, §9) — the third multiplier on the score line, after care and alongside tack. **This stage was missing from this list entirely until 6 Aug 2026**, despite being referenced by ten other documents, two engine signatures and a promised `show_entries` column; it is specified in `docs/slices/0027-training.md`. It belongs here for the same reason tack does — its prices are set against a market that has to exist first — and ahead of professions, because whether a trainer for hire is a feature or a hole in the action budget (§8c) is a question best answered after self-training has been watched in play. The trainer profession is deliberately not part of it.
