...
**Selling, and both buying routes, are now built (2026-08-03)** — `docs/slices/0017-market.md` Parts A, B and C. Sale listings are Part A's `appraise()` (§14 of that document has the exact formula, close to but not identical to this section's list — it adds a colour term via amendment 0017a and deliberately never reads pedigree, an open question left for a future session, §18). Buy offers are Part C's standing-offers board plus an NPC stable shopping the open market on its own tick cycle, both reusing the exact appraisal, target-scoring and sale path the rest of the market already uses — no second "how good is this horse" formula and no second sale batch. **Stud services (Part D) are the one piece of this section still entirely unbuilt.**
...
- **§10f** — **NPC selling and both buying routes are now built** (`docs/slices/0017-market.md` Parts B and C, 2026-08-03) — see this section's own update above. Stud services (Part D) remain unbuilt, so an NPC stable's population is still a closed line except for the operator's manual outcross batch on `/admin/npc` (§3.3 of that slice) — the point below is otherwise now out of date and kept only as history.
