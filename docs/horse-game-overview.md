...
**Update 4 Aug 2026:** all eight breeds' ideal vectors are now seeded (`migrations/0107`) and every breed-conformation show class exists for all eight, with the NPC show barn stocking each one to its own target (`docs/breed-ideal-vectors.md` §6, that date's build-log entry). The one piece of this paragraph still true as written: **the consignment dealer stocks Quarter Horses only** (`consignment_breed_codes` in config) — that was never blocked on a missing vector the way the show barn was, so it did not widen automatically alongside it, and remains a config edit away rather than a deploy whenever a session takes it up.
