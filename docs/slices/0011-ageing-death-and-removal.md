...
**Superseded on age-based decline, 3 Aug 2026 — raised as this section's own "what to raise rather than decide" note asked, and reversed. See `docs/slices/0014-before-the-children-play.md` §2.2/§2.1.** The operator was offered a gentle decline, a noticeable one, or nothing, and chose noticeable: about −15% at the extreme, applied as a new `ageModifier` multiplier alongside `careModifier`/`tackModifier`/`trainingFactor`, live-tunable, never touching the displayed conformation numbers. Read the rest of this section anyway — the argument about `realization()` in the third bullet is still correct and still binding, and it is *why* slice 0014 put the decline in the scorer rather than in expression. What changed is narrower than it looks: a last season is now a *harder* season, not a penalised one — the Failing marker itself still carries no penalty, because the new modifier is keyed off age in days, never off remaining life. A failing horse scored exactly what it would have scored the month before; a failing horse still does, unless it has also crossed the age-decline curve, which is a different question the same slice deliberately keeps separate (§2.2's whole argument).
