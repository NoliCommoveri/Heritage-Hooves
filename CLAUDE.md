...
| Health, first pass | specified, not built | **Next in the build order.** See `docs/slices/0010-health-first-pass.md`. Four disease loci for the Quarter Horse (HYPP and PSSM1 dominant, HERDA and GBED recessive), the `conditions` reference table, `horse_conditions` (truth) and `horse_knowledge` (what a stable has paid to learn), genotype tests bought from the horse page for money and a turn, a breeding-preview warning computed **only** from knowledge, HERDA barring a horse from showing, and GBED killing a foal 30 game days after it is born. Four decisions were taken when it was specified — see that document's §2, two of which close open questions in overview §3b. |
