...
| Care | built, in full (2026-08-03) | **Split out from what the design documents call "care and tack" on 2026-08-02 — tack is now its own stage, further down this table.** Built per `docs/slices/0013-care-and-condition.md`'s Part A: two care timers on the horse (farrier, vet wellness, `src/engines/care/modifier.ts`'s ramp-and-clamp), a barn-wide feed level on the stable, and the ±5% show-score modifier wired into `scoreEntry`/`scoreAbilityEntry`'s previously-pinned-at-1.0 `careModifier` parameter, snapshotted per result onto `show_entries.care_modifier_applied`. `/horses/:id/care` and `/stables/:id/care` (a one-click barn round) call the farrier or vet for money and **no turns**; `/stables/:id/feed` sets the barn's feed level, the lever a stable in debt can pull. `/admin/care` shows fresh/due/overdue counts, a care-modifier distribution, and a "make every horse overdue" testing control. **Part B (condition management for HYPP/PSSM1) built 2026-08-03 in `docs/slices/0014-before-the-children-play.md` §5**, keeping slice 0010 §3.2's promise of `horse_conditions.management_state`: an affected horse with no current plan carries `unmanaged_condition_penalty` (−0.03, stacks per condition) via a new `src/engines/health/management.ts`, wired into `careModifierForHorse`'s `conditionDelta` parameter (no longer pinned at 0) and the show-judging path (batched per class, no N+1 — `conditionDeltaMapForHorses` in `src/db/health.ts`). A plan costs `condition_management_cost` (150) for `condition_management_interval_game_days` (180), bought/renewed from a Management section on `/horses/:id/care` (shown only for a condition the viewing stable is entitled to know about — `ownerVisibleStatus` reused, never a second version of the rule) and a Management round button on `/stables/:id/care`. NPC show-barn horses are kept current by the tick alongside their shoes (`stampNpcHorsesCurrent`), never exempted at the scorer. `conditions.management_options` from slice 0010/0013's own sketch shipped as `conditions.management_text` (plain text) instead — see slice 0014 §7. Departs from `docs/horse-game-schema.md` §4.1 on the `care` JSON blob and the cached `care_modifier` (neither is built — see that slice's §2.1/§5.6). See the build log's 2026-08-03 care entry and its 2026-08-03 slice-0014 entry for what a future session needs to know. **The location flag (barn vs pasture) was added on top of this on 2026-08-03** — see the row below. |
| Location: barn vs pasture | built (2026-08-03) | Not from any slice document — asked for by the operator while care was being built, and specified in conversation. `horses.location` (`barn`/`pasture`) plus `horses.location_changed_game_day`, one pure engine (`src/engines/care/location.ts`'s `workAvailability`), one route (`POST /horses/:id/location`). A horse at pasture **still ages, still grows and still dies of old age on its rolled lifespan**, but its care timers are frozen, it costs no board (`pasture_upkeep_multiplier`, 0), and it cannot be bred or shown. Coming in credits the frozen days back onto both care timers, so a horse that goes out freshly shod comes home freshly shod, and starts a **settling period** (`pasture_settle_game_days`, 30) before it can be bred or shown again — that period is the whole reason free pasture is not free storage, so do not set it to 0 without replacing the trade-off. A mare in foal cannot be turned out at all. `checkEligibility` gained `at_pasture`/`settling_in` reasons and now takes a resolved `availability`; breeding checks the same function, so there is one implementation of the rule. See the build log's 2026-08-03 location entry. |
| Ageing and death | built (2026-08-02), decline curve added 2026-08-03 | `docs/slices/0011-ageing-death-and-removal.md`, taken in one session, both Part A and Part B. Horses die of old age on a lifespan **rolled once at birth and snapshotted** onto `horses.natural_death_game_day` (`src/engines/ageing/lifespan.ts`'s `rollLifespanGameDays`, `deriveSeed(rng_seed, 'lifespan')`) - not a hazard rolled every tick, which is what makes it idempotent, tunable and reproducible. Death arrives **announced**: a visible **Failing** marker for ~1.5 game years (`ageState`'s five states: young/adult/veteran/failing/ended), on the horse page, the barn list and a `horse_failing` event, closing `docs/horse-game-overview.md` §14's open question. The tick's new `assignLifespansAndNoticeFrailty` and `killDueOldHorses` stages (`src/db/ageing.ts`) sit between `killDueLethalFoals` and `createDueShows`. Voluntary removal ships alongside it as a free, one-way **retire away** (`/horses/:id/retire`, confirm-checkbox page) costing no turn and no money. Death and removal share one exit path, `buildEndHorseParticipationStatements` - cancels the horse's own in-progress pregnancy and booked covering (whichever table it appears in) and withdraws its entries in classes not yet judged. `/stables/:id/past` lists every horse a stable ever owned that has since ended; the barn list drops one `barn_shows_ended_game_days` after it ends. `/admin/ageing` is the new subpage (oldest living horses, recent deaths, a bring-forward testing control); `/admin/shows` gained the show barn's headcount-vs-target and its five oldest horses. Departs from `docs/horse-game-schema.md` §4.2 on two points, per its §5.5: `image_url` is kept, and `show_entries` are **not** pruned, because deleting them would retroactively falsify judged shows. **Age-based performance decline added 2026-08-03, `docs/slices/0014-before-the-children-play.md` §4, superseding this slice's own §3.2 which declined one** (raised again per that section's "what to raise rather than decide" note, and reversed). New `src/engines/ageing/performance.ts`'s `agePerformanceModifier`: flat at 1.0 below 16 game years, a straight line down to a floor of 0.85 at 25, keyed **only** off `gameDay - born_game_day` - never off `ageState`/the Failing marker, which is a function of remaining life, not age (§2.2's whole argument, and a test named for it). Wired as a fourth multiplier (`ageModifier`) alongside `careModifier`/`tackModifier`/`trainingFactor` on both `scoreEntry` and `scoreAbilityEntry`, snapshotted per result onto the new `show_entries.age_modifier_applied` column (mirrors `care_modifier_applied`, its own column rather than folded into care - the care clamp would otherwise eat it). Three new live tunables (`age_decline_start_game_days`, `age_decline_floor_game_days`, `age_modifier_floor`). Shown on the horse page (a line beside the age, silent when at prime), the result explanation (a second line beside Care's), the barn list (a quiet "Past its peak" badge, never a warning colour), and `/admin/ageing` (a prime/past-peak/floor distribution plus a modifier column on the oldest-horses table). NPC show-barn horses go through the identical scoring path - no special case, `CLAUDE.md` §13. |
...
| NPC stables | **built, in full (2026-08-03)** | Slice 0015, both parts. **Part A:** the data and the pure selection engine. Migrations `0083`-`0086`: `npc_policy` (one row per NPC stable — `target_kind`/`target_breed_id`/`target_discipline_code` rather than a free weights JSON, since the weights already live on `breeds.ideal_vector`/`disciplines.ability_weights`) and `npc_ceiling_schedule` (two columns, `conformation_ceiling` and `ability_ceiling`, since the two personality kinds cap different quantities on different scales — starter rows 76/52 rising to 88/64 over roughly nine game years, explicitly a guess); Cedar Hollow (conformation specialist) and Willow Creek Barrels (discipline barn, Barrel Racing) seeded as new empty NPC stables alongside a new `npc_policy` row for the existing Fair Meadow (now the "volume breeder" — high selection noise, short interval, expressed entirely through its policy row's numbers, no code branch on identity); NPC stables exempted from board (`chargeUpkeep` now skips `is_npc = 1`) with a one-off ledger-adjustment correction zeroing out what Fair Meadow had already accrued. `src/engines/npc/selection.ts`'s `selectBreedingPairs`/`clampedScore` reuse the judge's own `scoreEntry`/`scoreAbilityEntry` (`noise: 0`) rather than inventing a second "how good is this horse" formula — the ceiling clamps each trait's own *quality* (closeness-to-target for conformation, expressed value for ability) before re-aggregating with the scorer's own weights, never the raw expressed value or the aggregate. Also landed with Part A, beyond the slice's own split: `npc_policy` added to `src/db/reset.ts`'s `WORLD_ONLY_TABLES` (a full world reset otherwise fails outright, D1 enforcing the foreign key) and the world-reset re-seed block extended to recreate all three NPC stables and their `npc_policy` rows. **Part B, built same day:** the tick stage (`src/db/npcBreeding.ts`'s `runNpcBreedingDecisions`, wired into `src/db/tick.ts` immediately before `resolveDueCoverings`) reuses a player's own booking eligibility rather than a second version of it — `availabilityForHorse`, `isInBreedingSeason`, the age/recovery checks, `getActivePregnancyForMare`/`getBookedCoveringForMare`, and a new `loadPedigreeContextForMany` (`src/db/horses.ts`, the batched generalisation of `loadPedigreeContext` — one pedigree load for a whole candidate set rather than one per pair) feeding the same `coefficientOfInbreeding` a player's own COI warning uses. `npc_policy.last_bred_game_day` is the idempotency marker, in the same family as `last_upkeep_game_day` — set once per due cycle regardless of whether the cycle produced any pairs (debt, capacity, or nothing eligible all still advance it, so a stable recovering from debt waits for its *next* interval rather than firing on the very next tick). An NPC-bred foal is named at birth from its breeder's own prefix plus a word from the founding name-list (`src/db/npc.ts`'s `resolveUniqueNpcFoalName`, `buildFoalInsertStatements`' new optional `registeredName`) — a player's foal is completely unaffected, still born `NULL` for the owner to name. `judgeOneClass`'s show-field top-up now draws from every `is_npc = 1` stable's stock (`listNpcStableHorses`), not Fair Meadow alone — no other change to eligibility, seeding, or the zero-player-entries rule. `/admin/npc` is the new admin page: every NPC stable's roster/personality/last-bred/next-cycle-due/pairs-last-cycle, the ceiling schedule (edit or add a row, no deploy needed), a "found a new NPC stable" form (refuses a taken prefix rather than surfacing a raw constraint error), and an "add an outcross batch" control — the only way genetic material enters an NPC stable's closed population today, since breeding stays same-stable-only and each NPC stable is therefore its own closed line (a known, flagged limitation, not an oversight). `src/db/npc.ts`'s `stockShowBarn` (top-up-to-a-target, used by `/admin/shows`) and the new `stockNpcStable` (mints an exact count, used by the outcross control) now share one underlying `mintFoundingHorses` rather than either hardcoding a stable or a breed. Verified by hand end-to-end against a local `wrangler dev`: outcross-stocked Cedar Hollow and Willow Creek Barrels, advanced the tick past both stables' intervals, watched coverings book (round-robin confirmed — one stallion paired with more than one mare in a cycle) and a foal arrive named "Willow Creek Barrels Talon," edited the ceiling schedule and founded a fourth NPC stable live, and confirmed a full world reset restores exactly the original three. NPC buying/selling/stud services stay deferred to the Market stage regardless. See `docs/slices/0015-npc-stables.md` and the build log's 2026-08-03 entries.
| Market | not started | |
| Tack | not started, **not yet specified** | Split out of "care and tack" on 2026-08-02 and deliberately placed here, after the market: tack is a purchase economy (tiers with diminishing returns, wear with use, repair or replace), and every number in it is set against prices the market has not established yet. It also needs more than one discipline seeded to mean anything, since discipline specificity — a jumping saddle doing nothing in a gaited class — is most of its design. See `docs/horse-game-overview.md` §8b and `docs/slices/0013-care-and-condition.md` §3.1 for the full reasoning. `tackModifier` is already a parameter on both scorers, pinned at 1.0, so wiring it in later touches no formula. Tables: `tack_types`, `tack_items` (schema §8.5). **The pay-to-win risk in §8b is this stage's main design problem** — a wealthy player equipping everything at top tier widens gaps breeding alone would not. |
...
| Screens, the public world, and the admin door | built (2026-08-03) | Eight things the operator asked for in conversation, none of which changes a formula: filter tabs on the barn list (All/Mares/Stallions/Foals, plus a new display-only `foal_max_age_game_days`); class-type and breed filters on `/shows`; **a judged class no player entered stops being displayed** (a read-path filter only — judging, placings and prize money are untouched, per `CLAUDE.md` §13); a new **`/world` section** (`/world`, `/world/stables/:id`, `/world/horses/:id`) where every account can see every stable, its balance and its horses, with a read-only public horse page that carries name/breed/colour/age/picture/pedigree/show record and **never** a conformation number, health result, genotype or COI — `src/render/world.ts` never sees a `HorseRow`, only the plain view object `src/routes/world.ts`'s `buildPublicHorseView` assembles; the show placing screen linking the horse (to `/world/horses/:id`) and naming the entering stable (`listClassEntriesForDisplay` gained a stable/account join); `/admin/accounts` gaining rename, username change, an admin-flag toggle (guarded against removing your own flag or the last admin) and a delete that refuses any account owning a stable (active or inactive) or holding a founding-grant record, typed-word-plus-checkbox confirmed; and **a PIN gate in front of `/admin`**, built on slice 0005 §7.3/§7.4's own `accounts.pin_hash` + `pin_attempts` + global lockout (`src/lib/pin.ts`'s `decidePinAttempt`) so the deferred parent PIN inherits it rather than duplicating it — `/admin/security` sets/changes the PIN (own account only, password-confirmed), `/admin/unlock` is the gate and carries the §9.7 forgotten-PIN password path, and a `pin_hash IS NULL` account is nagged (a banner spliced in at the router level, `src/router.ts`'s `withAdminPinNag`) rather than locked (§9.6). Only the PIN touches the schema (migrations `0087`-`0089`, following slice 0015 Part A's `0083`-`0086`). `/admin/migrations` carries its own separately-implemented, fail-open PIN check (a hard-coded grace ceiling, not the config tunable — it can't read config). See `docs/slices/0016-admin-and-ui-improvements.md` and the build log's 2026-08-03 slice-0016 entry, including §9.6's two ways to lock the operator out permanently and how each is avoided, and §6.2.1 for the two things slice 0015 Part A changed under the `/world` page. **Deploy note:** apply the three new migrations at `/admin/migrations`, then visit `/admin/security` and set a PIN — until then, admin pages nag but still open, deliberately. **Two follow-ups landed the same day, from the operator in conversation** (see the build log's own follow-up entry): the PIN now re-locks the moment you leave `/admin`, not only after the time-based grace window elapses; and an admin viewing someone else's horse at `/horses/:id` now sees true health status (straight from the genotype, not filtered through any stable's knowledge) alongside the conformation/care/genotype detail admin already saw — plus a new `/admin/horses` name search, since going stable by stable to find one horse doesn't scale. |
| Robustness genes (polygenic health substrate) | built, substrate only, deliberately (2026-08-03) | `docs/slices/0014-before-the-children-play.md` §6, Part C of that slice - built ahead of everything else it needs, on purpose, because `TRAITS` (`src/engines/genetics/polygenic.ts`) is append-only and a genotype is written once at birth: a horse alive when a heritable trait is added is permanently missing it, and no later slice can fix that without a reset. Three traits appended after `agility` - `foot_robustness`, `joint_robustness`, `ligament_robustness`, ten loci each, category `hidden` (so both scorers and every display path exclude them automatically, same as `fertility`), direction `higher_better`. `ROBUSTNESS_TRAITS` (`src/engines/conformation/traits.ts`) names them for the one place that must treat them differently: `src/engines/founding/generate.ts`'s polygenic loop draws them at a fixed `robustness_one_chance` (0.5) regardless of a founding batch's quality band, so a top-band horse is not automatically sound as well as beautiful - soundness is an axis a player must breed for separately from the show ring. **Nothing reads these yet** - no onset roll, no risk display, no condition mapping, `horse_knowledge.kind` still only ever `'genotype'`. A horse page after this slice looks exactly as it did before. Migrations `0081`/`0082`. See the build log's 2026-08-03 slice-0014 entry for the append-only-`TRAITS` argument in full - the part a future session most needs and previously existed only as a code comment. |
