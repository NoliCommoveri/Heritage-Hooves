...
| Screens, the public world, and the admin door | specified, not built (2026-08-03) | Eight things the operator asked for in conversation, none of which changes a formula: filter tabs on the barn list (All/Mares/Stallions/Foals, plus a new display-only `foal_max_age_game_days`); class-type and breed filters on `/shows`; **a judged class no player entered stops being displayed** (a read-path filter only — judging, placings and prize money are untouched, per `CLAUDE.md` §13); a new **`/world` section** where every account can see every stable, its balance and its horses, with a read-only public horse page that carries name/breed/colour/age/picture/pedigree/show record and **never** a conformation number, health result, genotype or COI; the show placing screen linking the horse and naming the entering stable; `/admin/accounts` gaining rename, username change, an admin-flag toggle and a delete that refuses any account owning a stable; and **a PIN gate in front of `/admin`**, built on slice 0005 §7.3/§7.4's own `accounts.pin_hash` + `pin_attempts` + global lockout so the deferred parent PIN inherits it rather than duplicating it. Only the PIN touches the schema (migrations `0087`-`0089`, renumbered after slice 0015 Part A took `0083`-`0086` — check the directory before writing them). See `docs/slices/0016-admin-and-ui-improvements.md`, including §9.6's two ways to lock the operator out permanently and how each is avoided, and §6.2.1 for the two things slice 0015 Part A changed under the `/world` page. |
