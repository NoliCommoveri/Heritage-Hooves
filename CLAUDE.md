...
| One show class | specified, not built | The Quarter Horse's ideal vector and one class judged against it. Unblocked - slice 0006's expressed conformation values are what the scorer reads. See `docs/slices/0008-one-show-class.md`, which settles four things in conversation: shows run monthly (every 30 game days, closing the cadence open question), one NPC stable of real stored horses fills the field, **no money at all** (ribbons only - no entry fees, no prizes, no ledger), and three judges weighting the standard differently. Other breeds' ideals still come with the breeds stage below. |
