...
| The other seven breeds | **ideal vectors seeded, NPC show barn breed-aware (2026-08-04)** | Migration `0107` seeds all seven remaining `ideal_vector`s from `docs/breed-ideal-vectors.md` §3, so every show now carries eight breed classes. `stockShowBarn` (`src/db/npc.ts`) no longer hardcodes Quarter Horses - it tops every breed in play up to target independently, closing `docs/breed-ideal-vectors.md` §6.2's blocker. Still unbuilt for all eight: `eligible_class_types`/`discipline_aptitudes`, `height_range`/`weight_range`, and disease panels beyond the Quarter Horse's four (§6.3). |
