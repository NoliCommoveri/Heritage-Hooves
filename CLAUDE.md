...
| Consignment dealer (Part E) | built (2026-08-03), operator controls added (2026-08-04) | A dedicated NPC stable (`Consignment Yard`, no `npc_policy` row, no real balance) mints one or two Quarter Horses onto the market every 90 game days, at the mid quality band, via the tick's `runConsignments` stage. The operator can queue an allele from `/admin/consignment` to be seeded into the next batch (`consignment_injections`) — the only mechanism that introduces an allele nobody already owns. An injected locus is always pre-tested for the dealer. **2026-08-04:** `/admin/consignment` gained a "Mint a batch now" button (`forceConsignmentBatchNow`) that mints immediately, bypassing the cadence check, for when the operator wants a queued allele to feed the market the same day rather than waiting for the next scheduled batch. Separately, a queued injection now carries `eligible_from_game_day`: if a batch is already due/overdue at the moment of queuing (about to mint on the very next tick regardless), the injection is deferred to the batch *after* that one, so it's never swept into a batch the operator didn't mean to seed — "mint now" ignores this deferral on purpose, since it's an explicit override. See `docs/slices/0017a-amendment-colour-testing-and-the-consignment-dealer.md` §5. |
...
| The other seven breeds | not started, **ideal vectors and disease panels now drafted** | All eight breeds' ideal vectors are drafted in `docs/breed-ideal-vectors.md`; only the Quarter Horse's is live, blocked on the NPC show barn being Quarter-Horse-only. **2026-08-04:** disease panels drafted too, in `docs/breed-disease-panels.md` — six new single-gene conditions (SCID, cerebellar abiotrophy and lavender foal syndrome for Arabian; WFFS for German Warmblood; dwarfism and hydrocephalus for Friesian), with Thoroughbred, Paso Fino, Icelandic and Nokota deliberately getting none this pass (their real signature problems are polygenic, colour-linked-but-locus-missing, or — Nokota — genuinely absent). Unlike the ideal vectors, **this half has no show-barn blocker** and is seedable independently; nothing in `conditions`, `loci` or any `founding_allele_pool` has actually changed yet. |
