...
- **The person running this project does not write code.** They can read plain English, follow instructions, click deploy, and describe what looks wrong. They cannot debug. This means: never leave the repo in a non-deploying state, never hand back a change that requires manual steps not written down, and explain what you did in language that assumes no programming knowledge.
