...
| Market | **Parts A and B built (2026-08-03); C and D specified, not built** | Players can list, browse, withdraw and buy; everything travels with the horse, health is always disclosed, and the ledger's `counterparty_stable_id`/`same_account` columns finally have a writer. NPC stables now list their own surplus when near capacity, priced through the same appraisal every player's guide value uses, times a per-personality multiplier on `npc_policy`. **No NPC stable buys anything yet and there are no stud services** — the market's other two problems (no exit pressure from the buy side, no outcross route) are still open. See `docs/slices/0017-market.md` (§12-§13 for the unbuilt parts). |
...
| Consignment dealer (Part E) | built (2026-08-03) | A dedicated NPC stable (`Consignment Yard`, no `npc_policy` row, no real balance) mints one or two Quarter Horses onto the market every 90 game days, at the mid quality band, via the tick's `runConsignments` stage. The operator can queue an allele from `/admin/consignment` to be seeded into the next batch (`consignment_injections`) — the only mechanism that introduces an allele nobody already owns. An injected locus is always pre-tested for the dealer. See `docs/slices/0017a-amendment-colour-testing-and-the-consignment-dealer.md` §5. |
...
| Founding specialists (a breed winnable in 6-8 months) | **specified, not built (2026-08-03); §9's open questions need the operator before anyone builds** | Every founding horse arrives genuinely good at one conformation trait (near its breed's target) and one ability trait (high, with headroom), plus a doubled consignment rate. Measured with `docs/analysis/stable-timeline.mjs`: takes a casual child from 1.2 years and a 33% failure rate to ~200 days and 3%, on both the conformation and the discipline line. Only changes what NEW horses look like — no migration touches an existing horse. **Part A does nothing for the seven breeds with no seeded `ideal_vector`**, which is the real blocker on "eight breeds". See `docs/slices/0019-founding-specialists.md`. |
