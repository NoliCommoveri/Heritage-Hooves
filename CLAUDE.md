...
| Ageing and death | specified, not built | `docs/slices/0011-ageing-death-and-removal.md`. Horses die of old age on a lifespan **rolled once at birth and snapshotted** onto `horses.natural_death_game_day` (not a hazard rolled every tick — that shape is what makes it idempotent, tunable and reproducible). Death arrives **announced**: a visible Failing state for ~1.5 game years, on the horse page, the barn list and the events feed, closing `docs/horse-game-overview.md` §14's open question. Voluntary removal ships with it as a free, one-way **retire away** costing no turn. Departs from `docs/horse-game-schema.md` §4.2 on two points, per its §5.5: `image_url` is kept, and `show_entries` are **not** pruned, because deleting them would retroactively falsify judged shows. |
