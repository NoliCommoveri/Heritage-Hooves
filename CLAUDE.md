...
| Image slot | **next up**, not started | The image library and the per-horse picker. Unblocked 2026-08-03: the operator is generating the library per breed and uploading it through GitHub's web file editor, and overview §14's phenotype question is decided — **matched on breed only**, free choice within a breed's set. A numbered set per breed (`<breed code>-NN`) plus a `breeds.image_count` column, so the list is derived rather than listed and growing the library needs no code change. No slice document written yet. |
