...
| Colour/pattern loci and `head_profile` | **Parts A-B built (2026-08-04): schema and genetics only, no expression yet** | Ten new Mendelian loci (dun, silver, champagne, roan, tobiano, frame overo, splash, sabino1, leopard complex + PATN1, migrations `0113`-`0114`) and a fifth conformation trait, `head_profile`, judged by all eight breeds (migrations `0110`-`0112`). Grew out of two SQL drafts the operator wrote: the content was right, the dialect was Postgres against a SQLite database and both were data-only files for things needing engine code first. **Built so far**: `TRAITS`/`LOCI` extended, breed ideal vectors and judge weights patched via `json_set`, all eight breeds' founding pools re-seeded (`0114`, now the authoritative pool migration — `test/genetics/consistency.test.ts` and the two `poolJsonForBreed` test helpers all point at it, not `0051`). **Not built**: Part C (`expressPhenotype` — nothing yet turns these loci into a displayed colour), Part D (knowledge/prediction/market), Part E (the LWO lethal), Part F (the collapsed testing page), Part G (the world reset). No horse's visible colour has changed and the reset has not happened. Frame overo's `O/O` will be a real lethal through slice 0010's existing GBED path once Part E lands; dominant white is dropped; the new loci will be priced flat at the existing `genotype_test_cost`. Next session: `foalColourPossibilities` goes from 3 loci to 12 and must fold over phenotypes rather than genotypes or it blows the CPU ceiling (§6.2) — read that section closely before starting Part C. See `docs/slices/0021-colour-loci-and-head-profile.md` and the 2026-08-04 build-log entry for exactly what landed and two gotchas worth knowing (a bare `;` inside migration prose truncates a statement silently; the breed-ideal-vector tests now merge two migrations rather than parsing one). |
| Acquired conditions (colic, laminitis, and the rest) | **specified, not built (2026-08-04)** | A dozen breed-agnostic, non-genetic conditions — colic, choke, gastric ulcers, sporadic tying-up, strangles, hoof abscess/thrush, rain rot/mud fever, eye injury, laminitis, navicular, osteoarthritis, suspensory ligament injury — driven by care state, a workload proxy (recent show-entry frequency, pending real training), location, feed level (laminitis reads `feed_level` directly, giving Premium feed the real downside slice 0013 §13.2 flagged as missing), and a heritable robustness score for four of the twelve (`foot_robustness`/`joint_robustness`/`ligament_robustness`, the substrate slice 0014 built ahead of use and nothing has read until now). One onset-risk engine and one resolution engine drive all twelve rows, reusing slice 0014's management machinery and slice 0010's death path unchanged. **Operator decided, before this was written: an ignored acute condition carries real death risk (colic above all — 40% untreated vs 5% treated); only 4 of the 12 conditions carry any death risk at all**, the rest top out at a barred career or an ongoing management cost. See `docs/slices/0020-acquired-conditions.md`, including its own flagged discrepancy (§7.1's marker column needs a rename before build) and the numbers in §4 that most need real-play tuning. |
