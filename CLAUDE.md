...
| Care | built, Part A only (2026-08-03) | **Split out from what the design documents call "care and tack" on 2026-08-02 — tack is now its own stage, further down this table.** Built per `docs/slices/0013-care-and-condition.md`'s Part A: two care timers on the horse (farrier, vet wellness, `src/engines/care/modifier.ts`'s ramp-and-clamp), a barn-wide feed level on the stable, and the ±5% show-score modifier wired into `scoreEntry`/`scoreAbilityEntry`'s previously-pinned-at-1.0 `careModifier` parameter, snapshotted per result onto `show_entries.care_modifier_applied`. `/horses/:id/care` and `/stables/:id/care` (a one-click barn round) call the farrier or vet for money and **no turns**; `/stables/:id/feed` sets the barn's feed level, the lever a stable in debt can pull. `/admin/care` shows fresh/due/overdue counts, a care-modifier distribution, and a "make every horse overdue" testing control. **Part B (condition management for HYPP/PSSM1, keeping slice 0010 §3.2's promise of `horse_conditions.management_state`) was not built** — deferred per the slice's own §12 scope-cut; `conditionDelta` exists as a parameter on the engine and is always 0. Departs from `docs/horse-game-schema.md` §4.1 on the `care` JSON blob and the cached `care_modifier` (neither is built — see that slice's §2.1/§5.6) and is also where a decline-with-age performance modifier belongs when one is wanted (slice 0011 §3.2). See the build log's 2026-08-03 care entry for what a future session needs to know, including Part B's remaining scope. **The location flag (barn vs pasture) was added on top of this on 2026-08-03** — see the row below. |
