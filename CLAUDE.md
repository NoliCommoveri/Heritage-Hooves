...
| Discipline shows | specified, not built | Six disciplines (barrel racing, flat racing, show jumping, endurance, dressage, gaited pleasure) judged on ability rather than conformation, plus a fifth ability trait, `agility`. Reuses slice 0008's calendar, eligibility, placings, noise and prizes — `checkEligibility` already handles `requires_gait`/`crosses_eligible` and needs no change. **Ordered before NPC stables and market by the operator, 2026-08-02.** **Has a hard precondition: it must not deploy without the world reset** — see `docs/slices/0011-discipline-shows.md` §4.2, where the failure is silent. |
