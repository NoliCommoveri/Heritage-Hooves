...
| Colour/pattern loci and `head_profile` | **Parts A-C built (2026-08-04): schema, genetics and expression all land; not yet visible in knowledge/prediction/market** | Ten new Mendelian loci (dun, silver, champagne, roan, tobiano, frame overo, splash, sabino1, leopard complex + PATN1) and a fifth conformation trait, `head_profile`, judged by all eight breeds. `expressPhenotype` now turns those loci into a real displayed colour and an ordered `patterns[]` list — dun/champagne/silver compose into named colours (grullo, amber champagne, silver bay, ...) with a fixed adjective order as a fallback, roan prefixes the result, and frame overo/splashed white each have a configurable penetrance (`config.values.pattern_penetrance`) so a carrier isn't always visibly marked. **Not built**: Part D (knowledge/prediction/market - `foalColourPossibilities` still only covers three loci), Part E (the LWO lethal), Part F (the collapsed testing page), Part G (the world reset). No world reset has happened yet, so no existing horse has a `head_profile` genotype or any of the ten new loci - see slice §1 for why a reset is required before this is playable. See `docs/slices/0021-colour-loci-and-head-profile.md` and `docs/build-log.md` for what's landed so far. |
