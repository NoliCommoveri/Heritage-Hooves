...
| Care | specified, not built | **Split out from what the design documents call "care and tack" on 2026-08-02 — tack is now its own stage, further down this table.** Specified in `docs/slices/0013-care-and-condition.md`: two care timers on the horse (farrier, vet wellness), a barn-wide feed level on the stable, and one pure engine (`src/engines/care/`) turning them into the ±5% show-score modifier `scoreEntry`/`scoreAbilityEntry` already carry as a pinned-at-1.0 parameter. Care calls cost money and **no turns**, with a one-click barn round, and feed is the lever a stable in debt can pull. Part B keeps slice 0010 §3.2's promise of `horse_conditions.management_state`. Departs from `docs/horse-game-schema.md` §4.1 on the `care` JSON blob and the cached `care_modifier` (neither is built — see that slice's §2.1/§5.6) and is also where a decline-with-age performance modifier belongs when one is wanted (slice 0011 §3.2). |
