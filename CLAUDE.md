...
| Turns and tick | specified, not built | **Next in the build order.** See `docs/slices/0009-turns-upkeep-and-the-ledger.md`. The world clock, the tick slots and the time scale landed in slice 0001 and the tick has since grown breeding and show stages, so what is left is the money and the turns: the `ledger`, per-horse upkeep charged on the world clock, **show prize money** (reversing slice 0008 §2.3), the per-account action budget, `/admin/money` for adding money by hand, and the `events` feed. Four decisions were taken when it was specified — see that document's §2. |
