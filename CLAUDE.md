...
| Incidents untangled from genetics, and conformation labels | **specified, not built (2026-08-04)** | Two unrelated pieces of work written up together because the operator raised them together; they share no code and can be built separately. **Part A** corrects a structural mistake in slice 0020: the twelve acquired conditions were filed in `conditions`/`horse_conditions`, the genetics tables, where `locus_code`, `severity_class`, `test_cost_key`, `breed_associations` and `signs_visible` are all dead weight or a lie for them — and where six separate call sites now have to remember a `category !== 'acquired'` filter to stop colic appearing on the testing page with a price next to it. The fix moves them to their own `incident_types`/`horse_incidents` tables and **deletes all six filters** rather than correcting them. No number changes; the risk engine, tick stages, treatment, death path and show barring all keep today's behaviour. The one real tie to genetics stays and is not a mistake: four of the twelve read a heritable robustness trait as an input to onset *risk*, which is never a test result and never a carrier status. **Part B** puts a one-word verdict (Poor / Weak / Acceptable / Good / Outstanding) next to each conformation trait once `horse_show_summary.starts >= 1`, computed from the judge's own per-trait formula against the horse's own breed's `ideal_vector` — finally keeping the Conformation card's own closing promise that "which end a breed wants arrives with the first show class." The bars must be shortened into a flex row with `min-width: 0` so the word doesn't overflow off a phone screen. **Operator decided both of the questions this spec opened.** The incident history is trimmed to a display-only window (`incident_history_game_days`, default 720 = two game years) measured from `resolve_game_day` — rows are never deleted, because `/admin/incidents`'s outcome distribution is the only instrument that says whether slice 0020's untuned numbers are right; open and `degenerative` incidents ignore the window, and a hidden count is always stated. The breeding preview gets the labels too, words-only and per parent against **each parent's own breed** ideal (cross-breed pairings are possible), since deciding a pairing is the reason the labels exist. A horse with no starts reads **Unknown** rather than rendering nothing — a blank column reads as either a bug or an answer, and *Unknown* also tells a child what to do about it. See `docs/slices/0022-incidents-untangled-and-conformation-labels.md`. |
