...
| Conformation (one polygenic trait) | specified, not built | Expression and display for the four conformation traits, which every horse already carries and nothing shows. Brought forward ahead of the image slot — see `docs/slices/0006-conformation.md` §9 for why. |
| Image slot | not started | Moved behind the show class by slice 0006 §9: it is blocked on an image library the operator has no terminal to produce, and overview §14's phenotype-matching question is easier to answer once a structured phenotype exists. |
