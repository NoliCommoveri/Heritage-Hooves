...
| Market | **Part A built (2026-08-03); B, C and D specified, not built** | Players can list, browse, withdraw and buy; everything travels with the horse, health is always disclosed, and the ledger's `counterparty_stable_id`/`same_account` columns finally have a writer. **No NPC stable lists or buys anything and there are no stud services**, so with five players the market may simply sit empty — that is Parts B and C being missing, not a bug in A. See `docs/slices/0017-market.md` (§11-§13 for the unbuilt parts). |
