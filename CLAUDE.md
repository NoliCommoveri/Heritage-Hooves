...
| NPC solvency | **built (2026-08-04)** | NPC stables only show when a player enters the same class and only take the slots the players left, which made prize money — the game's only recurring faucet — a function of how much the children happen to show. `src/db/npcFinance.ts` gives them two income sources instead. **The income floor** (`npc_policy.balance_floor`, migration `0118`): once every `npc_balance_floor_interval_game_days` a stable below its floor is topped back up to it, and a stable that is earning gets nothing — a safety net against a stable going permanently silent, not an operating budget. Its marker advances whether or not money moved, which is the property that stops it becoming an unlimited money pump; do not move that update back inside the `if`. **Off-screen clearance**: an NPC listing that runs its full window unsold is bought by somebody outside the game at `npc_listing_clearance_fraction` of guide value and the horse leaves via slice 0011's own retire-away path — this is where an NPC's money is meant to come from, since it only pays for horses good enough to be worth something, and every player had the full listing window to buy first. Also fixes a papercut: a broke NPC used to keep a standing buy offer up at the `market_min_value` floor, and now goes dark below `npc_buy_offer_min_balance`. All the numbers are first guesses; `/admin/npc`'s floor columns are how the operator watches them. See `docs/build-log.md`, 2026-08-04. |
