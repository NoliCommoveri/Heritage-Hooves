...
| Image slot | **next up**, specified, not started | The image library and the per-horse picker. Unblocked 2026-08-03: the operator is generating the library per breed and uploading it through GitHub's web file editor, and overview §14's phenotype question is decided — **matched on breed only**, free choice within a breed's set. A numbered set per breed (`<breed code>-NN`) plus a `breeds.image_count` column, so the list is derived rather than listed and growing the library needs no code change. See `docs/slices/0007-image-slot.md`. |
| Conformation (one polygenic trait) | specified, not built | Expression and display for the four conformation traits, which every horse already carries and nothing shows. See `docs/slices/0006-conformation.md`. Built **after** the image slot — an earlier draft of that slice argued the reverse and §9 of it now records the withdrawal. Nothing technical couples the two; either could be built alone. |
| One show class | not started | The Quarter Horse's ideal vector and one class judged against it. Blocked on slice 0006 — a scorer needs an expressed value to score. Other breeds' ideals come with the breeds stage below. |
