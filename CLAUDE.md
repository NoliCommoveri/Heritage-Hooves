...
| NPC stables | specified, not built | Three personality stables (a conformation specialist, a discipline barn, and the existing Fair Meadow show barn repurposed as a volume breeder), a `src/engines/npc/selection.ts` policy that reuses the judge's own `scoreEntry`/`scoreAbilityEntry` to rank a stable's own stock, and a `game_day`-scheduled quality ceiling that clamps expressed trait values before scoring (never the horse itself) — the concrete answer to overview §10d's escalation risk. New tables `npc_policy` and `npc_ceiling_schedule`, per schema §9/§12's build order; NPC buying/selling/stud services stay deferred to the Market stage. Breeding stays same-stable-only for now, so each NPC stable is its own closed population until a market exists — a known, flagged limitation, not an oversight. See `docs/slices/0015-npc-stables.md`.
| Market | not started | |
