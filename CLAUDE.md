...
| Colour/pattern loci and `head_profile` | **specified, not built (2026-08-04)** | Ten new Mendelian loci (dun, silver, champagne, roan, tobiano, frame overo, splash, sabino1, leopard complex + PATN1) and a fifth conformation trait, `head_profile`, judged by all eight breeds. Grew out of two SQL drafts the operator wrote: the content was right, the dialect was Postgres against a SQLite database and both were data-only files for things needing engine code first. **Operator decided before it was written: a full world reset** (so no legacy-horse stand-in is needed for the new trait), **frame overo's `O/O` is a real lethal** through slice 0010's existing GBED path, **dominant white is dropped**, and the new loci are priced flat at the existing `genotype_test_cost`. Carries three things a session must not miss: `parseAllelePool` throws for every breed the moment `LOCI` grows, so the founding-pool migration is mandatory in the same push; `foalColourPossibilities` goes from 3 loci to 12 and must fold over phenotypes rather than genotypes or it blows the CPU ceiling; and the testing page collapses into default-closed `<details>` groups. See `docs/slices/0021-colour-loci-and-head-profile.md`. |
