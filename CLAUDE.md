...
| Pregnancy, heat and fertility | built (2026-08-03), display fix 2026-08-04 | Booked coverings, an oestrous cycle, a conception roll, real gestation, and twins. See `docs/slices/0003-breeding-and-fertility.md`. **2026-08-04:** every refusal was already enforced correctly at the point of action, but three screens invited a click that was always going to be refused - a filly's own page read "In season now.", the barn list badged her the same way, an immature colt's page offered him at stud, and the Breed/`/market/stud` mare and stallion pickers listed horses that could not actually be bred. `isOldEnoughToBreed` (`src/engines/breeding/maturity.ts`) and `mareIneligibility`/`stallionIneligibility` (`src/engines/breeding/eligibility.ts`) now answer the same question `validateBooking` and `validateStudBooking` already refuse on, reused rather than restated a third time. See `docs/fixes/breeding-eligibility-display.md` and `docs/build-log.md`, 2026-08-04. |
...
| Image slot | built (2026-08-02), operator picture labels added (2026-08-04) | Image library and per-horse picker; no image assigned at birth. See `docs/slices/0007-image-slot.md`. **2026-08-04:** `/admin/breeds` gained an optional text caption per numbered picture (`breed_image_labels`, migration `0138`), shown under the thumbnail in the picker so the operator can tell pictures apart when picking - describes the picture, never the horse it ends up on. See `docs/build-log.md`, 2026-08-04. |
...
| NPC solvency and the pet home | **built (2026-08-04)** | NPC stables only show when a player enters the same class and only take the slots the players left, which made prize money — the game's only recurring faucet — a function of how much the children happen to show. Two things hold them up. **The income floor** (`npc_policy.balance_floor`, `src/db/npcFinance.ts`, migration `0126`): once every `npc_balance_floor_interval_game_days` a stable below its floor is topped back up to it, and one that is earning gets nothing — a safety net against going permanently silent, not an operating budget. Its marker advances whether or not money moved, which is what stops it becoming an unlimited money pump; do not move that update back inside the `if`. **The pet home** (`src/db/petHome.ts`): one mechanic, two entry points, one price (`pet_home_payout_fraction`, 0.4 of appraised value) and one ledger kind (`pet_home_payout`). A player chooses it from the horse page; an NPC gets there only by a listing running its full window unsold. **The two are deliberately asymmetric — nothing automatic can ever send a child's horse away, and an NPC never "decides" to use one.** A departing horse that never showed and never conceived is deleted outright rather than kept as a `removed` row (`src/db/horseRemoval.ts`, whose `deletableHorseSql` is both the operator's rule and the proof the delete cannot break a foreign key — a new table referencing `horses` must be added to one of its two lists). Bred means pregnant, not covered. **2026-08-04, operator-reported: that delete had never once fired for a home-bred or founding horse.** Two of the three clauses the file's own comment called "defensive rather than expected" were universal — every foal born in a player stable gets a `foaled` event pointing at it, and every founding horse gets an `import_candidates.horse_id` — so between them the rule excluded almost the entire population it was written for. Both pointers are now NULLed by `buildDeleteHorseStatements` instead of blocking the delete (the operator's call: keep the feed's words, drop the link — every event sentence renders from its own `payload` and never joins `horses`). `consignment_injections` stays a keep-clause on purpose: rare rather than universal, and the only one of the three actually rendered anywhere. Alongside it, `removedHorseLabel` (`src/render/horses.ts`) makes all four screens that name an ended horse read `end_reason` — three of them called every pet-home horse "Retired away". See `docs/build-log.md`, 2026-08-04. Also fixes a papercut: a broke NPC used to keep a standing buy offer up at the `market_min_value` floor, and now goes dark below `npc_buy_offer_min_balance`. All numbers are first guesses. See `docs/build-log.md`, 2026-08-04. |
...
| NPC stables for Paso Fino and German Warmblood | **built (2026-08-04)** | Confirms the spec's own finding: since slice 0015 built every downstream NPC mechanism to read any breed/discipline generically, this landed as two migrations (`0136`/`0137`) and one `src/db/reset.ts` refactor, no engine/tick/route changes. The operator named all nine personality stables and asked for full parity (three personalities per breed, including a volume breeder, for all three breeds) rather than the spec's two-per-breed recommendation. The three existing Quarter Horse stables were renamed (Fair Meadow → Apples and Oats Ranch, Cedar Hollow → Bronco Valley, Willow Creek Barrels → Horseshoe Bay); six new stables were added for Paso Fino (Carrot Cove/conformation, Honeycomb Hills/Gaited Pleasure, Reese Puff Horses/volume) and German Warmblood (Queens Palace/conformation, Springen Stables/Show Jumping, Suchon Acres/volume) — every personality's numbers copied verbatim from its Quarter Horse counterpart. German Warmblood's discipline was read from the operator's own stable name ("Springen" = German for "jump") rather than asked separately; flagged for correction if that reading is wrong. See `docs/slices/0023-npc-stables-paso-fino-and-german-warmblood.md` for the original spec and `docs/build-log.md`, 2026-08-04, for what actually landed and departed from it. |
| Market | **All four parts built (Parts A-C 2026-08-03, Part D 2026-08-04)** | Players can list, browse, withdraw and buy; everything travels with the horse, health is always disclosed, and the ledger's `counterparty_stable_id`/`same_account` columns finally have a writer. NPC stables list their own surplus when near capacity (Part B), and buy too (Part C): a standing offers board (`buy_offers`, one active offer per NPC stable, updated in place each tick by `refreshNpcBuyOffers`) that a player sells into instantly from `/market`'s Offers tab, and a tick stage (`runNpcMarketPurchases`) where each NPC stable shops the open market itself within its own target, budget and free-stall buffer. Both routes reuse the exact appraisal, target-scoring and sale batch the rest of the market already uses — no second sale path anywhere. NPC balances are real and never topped up automatically; `/admin/npc` shows each stable's balance next to what it has spent buying and earned selling this game year, so a stable going quiet is visible before the children notice. **Stud services (Part D) are now built**: `stud_listings`/`stud_bookings` (`src/db/stud.ts`), a `/market/stud` browse-and-book flow reached from the horse page's "Offer at stud" card, and the first cross-stable breeding in the game — a mare's owner books to another stable's stallion without either side giving up a horse or a stall. Operator decisions taken when this part was commissioned: no live-foal guarantee (a missed conception is not refunded, same as an ordinary same-stable booking), and the market's commission applies to a stud fee exactly as it applies to a sale (two new ledger kinds, `stud_fee_paid`/`stud_fee_received`, alongside the existing `commission`). NPC stallions stand at stud too (`src/db/npcStud.ts`), reusing `bookStud` unchanged — the one new mechanism is that an NPC stallion is withdrawn from stud the moment his quality crosses the NPC ceiling (slice 0015 §2.4), since a booking combines his genetics with a player's own stock in a way an outright NPC purchase never does. See `docs/slices/0017-market.md` §13. **2026-08-04, two operator-reported defects:** a stud booking refusal used to replace the whole booking block, mare picker included, so a player with four mares whose first one was in foal had no way to say "not that one, this one" - the picker is now its own GET form back to `?mare=<id>` and renders refusal or not. And the Breed page gained **a third picker for every stallion standing at another ranch** (NPC ranches included), so cross-stable breeding is reachable from the page a child goes to in order to breed, rather than only from `/market/stud`. Its Book button posts to `/market/stud/:id/book` unchanged - still exactly one cross-stable breeding path (§13). Its options read `<breed> - <name> at <ranch>. Fee <n>.` and are ordered by the mare's own breed first, then the other breeds alphabetically - ordering only, never filtering. The preview is built by one shared `buildBreedPreview`, whose only two options are knowledge-boundary decisions: an outside stallion's health reads from **his owner's** knowledge rows (exactly the market's existing disclosure, no wider) and his conformation reads **Unknown** (it is shown nowhere else in the game to anyone but his owner). See `docs/build-log.md`, 2026-08-04. **2026-08-04, a third operator-reported defect: the market would not sell a horse between two barns one child runs**, which §13 makes the *only* route between them — so the horse could not move at all. `listingBuyRoute` had always allowed it (its own §7.1 step 2 comment, its stable-scoped refusal, `sellListing`'s `sameAccount` flag, and a passing test since slice 0017); `listingPageRoute` computed `buyerOptions = isMine ? [] : ...` and never drew the form. Now built for every viewer: your own listing renders the withdraw card *and* a buy card when you have another stable. It stays a real sale — full price, commission, one turn, `ledger.same_account = 1`, which slice 0017 §2.2 built to make same-account trading visible rather than to forbid it. **This was the second "handler with no way to reach it" in two days** (the `/treat` 404 was the first), so `test/market/same-account-buy.test.ts` guards it the way `test/router-paths.test.ts` guards the router — by reading the source, not by restating the rule. |
...
| The other seven breeds | **ideal vectors seeded, NPC show barn breed-aware (2026-08-04); disease panels built (2026-08-05)** | Migration `0107` seeds all seven remaining `ideal_vector`s from `docs/breed-ideal-vectors.md` §3, so every show now carries eight breed classes. `stockShowBarn` (`src/db/npc.ts`) no longer hardcodes Quarter Horses - it tops every breed in play up to target independently, closing `docs/breed-ideal-vectors.md` §6.2's blocker. Still unbuilt for all eight: `eligible_class_types`/`discipline_aptitudes`, `height_range`/`weight_range`. **Disease panels, drafted 2026-08-04 in `docs/breed-disease-panels.md` and built 2026-08-05 per `docs/fixes/breed-disease-panels.md`**: seven new loci (SCID/CA/LFS for Arabian, WFFS for German Warmblood, DWARF/HYDRO for Friesian, plus DSLD for Paso Fino - not in the original design doc, added because that document left Paso Fino with nothing) and MCOA for Icelandic, reading the existing Silver locus a second way rather than a new one. Prompted by an operator report that every breed's testing page offered the same five Quarter Horse tests regardless of what a horse could actually carry - `conditions.breed_associations` had always been display-only (migration `0052`'s own comment), so nothing had ever narrowed the offered list. The real fix is `panelFor` (`src/engines/health/panel.ts`), a pure function that derives a horse's panel from `founding_allele_pool` frequencies across its own breed plus every breed in its pedigree (`pedigreeBreedCodes`, `src/db/health.ts`, via `horse_ancestors` - ancestry, not just `breed_id`, because a Quarter Horse x Arabian foal can carry SCID) - never from the hand-maintained `breed_associations` column, which stays display-only and is now itself checked against the pools by a test. Wired into the seven offer-and-disclosure call sites the fix document named (the test page, the barn's Health card, a market listing's disclosure, the breeding preview's warning line - unioned across both parents, the NPC market-listing panel purchase, and the consignment dealer's pre-test); truth (`buildHorseConditionStatements`, `killDueLethalFoals`, `isBarredFromShowing`, `visibleAffectedConditions`, `getLethalTriggers`, `conditionCensus`) stays unfiltered on purpose, since a genotype's status never depended on what is worth testing for. Also widens PSSM1's pool to German Warmblood and Paso Fino (a real, not QH-exclusive, myopathy), which is what gives those two breeds a second panel condition apiece with no invented locus - and required a follow-up migration (`0152`) correcting PSSM1's now-stale `breed_associations`, caught by the new consistency test. Hydrocephalus is seeded `degenerative`, not `lethal` as the design document's own §5.6 specified - an operator decision recorded in the fix document, keeping the game's lethal count at five rather than six against overview §3b's own ceiling. Separately, **a condition's signs now take time to show**: `signs_visible = 1` used to mean visible the instant a horse existed, so a newborn foal was flagged with HERDA on its birthday. `conditions` gained `signs_delay_min/max_game_days` (100-250 as the default band for HYPP/PSSM1/HERDA, 20-60 for CA, 250-400 for DSLD, 0 for everything genuinely visible at birth) and `horse_conditions` gained `signs_game_day` (drawn once from the horse's own `rng_seed`, never `Math.random()`) and `signs_noticed_game_day`; a new tick stage, `noticeDueConditionSigns`, writes the `condition_signs` event once the delay elapses instead of at birth, and `isBarredFromShowing`/`visibleAffectedConditions` both follow the signs rather than the raw genotype. The admin create-horse form's loci list - now twenty-six fieldsets - is grouped into collapsed sections by category, deliberately left unfiltered by breed (a neutral control, slice 0005 §6.6), with a muted line explaining that an allele set outside its breed will be real but untestable. See `docs/fixes/breed-disease-panels.md` for the full brief and `docs/build-log.md`, 2026-08-05, for what actually landed. |
| Screens, the public world, and the admin door | built (2026-08-03), **show record unified (2026-08-04)** | Barn/show filters, a read-only `/world` public section, account admin, and a PIN gate in front of `/admin`. See `docs/slices/0016-admin-and-ui-improvements.md`. **2026-08-04:** the Show record card had drifted into two implementations - the owner's own horse page grouped placings by class type (Conformation, Endurance, Show Jumping, ...), while the three screens showing *somebody else's* horse (a sale listing, a stud listing, and `/world/horses/:id`) showed one flat list of five placings labelled with the show's made-up name and no class type at all. `buildShowResultGroups`/`showResultGroupsHtml` (`src/render/shows.ts`, lifted verbatim out of `routes/horses.ts` so the owner's card is unchanged) are now the single builder and renderer all four use, and the three non-owner screens fetch `SHOW_RESULT_FETCH_LIMIT` (200) rows instead of 5 because the cap is per group, not overall. See `docs/build-log.md`, 2026-08-04. |
...
| Acquired conditions (colic, laminitis, and the rest) | **built (2026-08-04)** | A dozen breed-agnostic, non-genetic conditions — colic, choke, gastric ulcers, sporadic tying-up, strangles, hoof abscess/thrush, rain rot/mud fever, eye injury, laminitis, navicular, osteoarthritis, suspensory ligament injury — driven by care state (derived from the existing farrier/wellness timers' own deltas, not a second ramp), a workload proxy (recent show-entry frequency), location, feed level (laminitis reads `feed_level`'s upkeep multiplier directly, giving Premium feed the real downside slice 0013 §13.2 flagged as missing), and a heritable robustness score for four of the twelve (`foot_robustness`/`joint_robustness`/`ligament_robustness`, the substrate slice 0014 built ahead of use and nothing had read until now). One onset-risk engine and one resolution engine drive all twelve rows from data in what was, at the time this row was written, `conditions.trigger`; the tick's `rollAcuteIncidents`/`resolveAcuteIncidents` sit right after `noticeCareDue`. (Slice 0022 Part A later moved this engine and its data to `src/engines/incidents/risk.ts`/`incident_types.risk_model` — see that row below — with no change to the behaviour described here.) A `manageable` outcome reuses slice 0014's management machinery unchanged; a `death` outcome reuses `buildEndHorseParticipationStatements` (the fuller adult-horse exit path, not `killDueLethalFoals`'s neonatal one — an acquired-condition death can hit a horse with live coverings/pregnancies/show entries, which a 30-day-old GBED foal never has) and deliberately does **not** also write a `horse_died` event, since that event's render is worded for a newborn foal and would misrender at any other age; `incident_resolved`'s own death-outcome sentence carries the narrative instead. Two new show-eligibility reasons (`acute_incident`, `degenerative_incident`) and a second contributor to `careModifier`'s `conditionDelta` slot (a flat penalty while any incident is open, §2.9). New card on the horse page (Incidents, between Health and Care), a barn-list badge, `/horses/:id/treat`, and `/admin/incidents` (open counts, outcome distribution, a force-incident testing control). **Operator decided, before this was written: an ignored acute condition carries real death risk (colic above all — 40% untreated vs 5% treated); only 4 of the 12 conditions carry any death risk at all**, the rest top out at a barred career or an ongoing management cost. **Two of the slice document's own outcome tables didn't sum to 1.0** (Strangles: 0.85/0.95; Eye injury: 0.95) — caught by the test the slice itself specified (§10 test 4) and corrected by raising each `resolved` share to close the gap; see `docs/build-log.md`. **2026-08-04: the "Call the vet" button had never worked.** `POST /horses/:id/treat` was added to the router's handler chain and never to `HORSE_ROUTE`'s own list of allowed sub-paths, so every press fell past the horse block to the router's closing `notFound()` - a bare "Not found" page in front of a handler that was otherwise correct. Fixed, and `test/router-paths.test.ts` now reads `src/router.ts` itself and asserts the regex and the handler chain still agree. See `docs/slices/0020-acquired-conditions.md` for the full design and the numbers in §4 that most need real-play tuning at `/admin/incidents`. |
| Breed ability bias and discipline aptitude | **built (2026-08-04)** | Operator report: German Warmbloods were beating Quarter Horses in barrels. Nothing was broken - **breed had never entered ability at any point**. `potential()` -> `geneticValue` -> `expressedValue` -> `scoreAbilityEntry` referenced `breed_id` nowhere; `founding_allele_pool` is Mendelian only, `ideal_vector` is conformation only, and a discipline `show_class` carries `breed_id = NULL` by migration `0064`'s own CHECK. Classes were decided by whoever rolled a slice 0019 founding specialist in `speed`/`agility` - a flat 2-in-5 chance for every breed alike. Two mechanisms now, both requested after the trade-off was shown. **`breeds.ability_bias`** (migrations `0141`/`0142`, `src/engines/breeds/identity.ts`) offsets the quality band's `polygenic_one_chance` inside `generateCandidate` - heritable, breeds true, reaches only horses generated afterwards; **every row sums to zero**, the constraint that makes a breed different rather than better. **`breeds.discipline_aptitudes`** (migrations `0143`/`0144`) multiplies a discipline class's final score - reaches every existing horse at once, but no amount of good breeding answers it; rows are deliberately not zero-sum, so the check instead is that each row's mean sits within 0.02 of 1.00. Both tests read the seed migrations off disk rather than hand-copying the numbers. **±0.05 was chosen because a specialist OVERWRITES its trait's potential rather than adjusting it** - so a breed's bias never touches the one trait a horse was singled out for, and a Warmblood with a speed specialist still takes a barrel class from an ordinary Quarter Horse (57.4 vs 56.9) while an ordinary Warmblood does not. A crossbred horse always reads neutral (its `breed_id` names the registry it competes under, not its ancestry). The aptitude is a **live tunable and is NOT snapshotted** onto the class - there is no `breed_id` on a discipline class to snapshot it against - so a discipline entry's `score_breakdown` carries an `aptitude` block as the only surviving record of what a horse was judged with. `scoreEntry` gets no counterpart and must not grow one. Separately, on the operator's decision, **NPC-minted horses now get founding specialists too**: `mintFoundingHorses` had passed neither the ideal vector nor the eligible ability traits since slice 0019, so player stock was systematically better than the field it competed against and nothing had written that down - this raises NPC quality, so watch `/admin/npc`. Horses created by hand at `/admin/horses` still get none of it, on purpose (a flat 50/50 draw makes a neutral control, slice 0005 §6.6). See `docs/breed-ability-and-aptitude.md`. |
| Discipline judges | **built (2026-08-04)** | Answers slice 0012 §15's open question: a discipline class now has judge variance, the way a breed class always has. `judges` gained a `kind` column (`conformation`/`discipline`, plain `ALTER TABLE ADD COLUMN`, no CHECK — the pairing precedent is `breeds.ideal_vector`, migration `0034`, not a rebuild) and `ability_weights` (migration `0139`); three new discipline judges seeded (migration `0140`) alongside the three existing conformation ones. `createShowIfMissing` draws a class's judge from the matching pool now, not one shared pool — the `class_N`/`judge_N` seed derivation itself is unchanged. `scoreAbilityEntry` gained an optional `judgeWeights` parameter (defaults to `{}`, every pre-existing caller unaffected) folding a judge's opinion into a discipline's own weight exactly as `scoreEntry` already folds one into `ideal.weight`. Also fixes a standing cosmetic bug: a discipline class's result page used to display a conformation judge's blurb, which had zero effect on the score shown beneath it. See `docs/slices/0024-discipline-judges.md`. |
| Incidents untangled from genetics, and conformation labels | **built (2026-08-04)** | Two unrelated pieces of work written up together because the operator raised them together; they share no code and were built together anyway. **Part A** corrects a structural mistake in slice 0020: the twelve acquired conditions were filed in `conditions`/`horse_conditions`, the genetics tables, where `locus_code`, `severity_class`, `test_cost_key`, `breed_associations` and `signs_visible` were all dead weight or a lie for them — and where six separate call sites had to remember a `category !== 'acquired'` filter to stop colic appearing on the testing page with a price next to it. Moved to their own `incident_types`/`horse_incidents` tables (migrations `0129`-`0133`, the last one backfilling every open incident before deleting the twelve old rows) and **all six filters deleted** rather than corrected. No number changes; the risk engine (renamed `src/engines/incidents/risk.ts`, was `src/engines/health/acquired.ts`), tick stages, treatment, death path and show barring all kept today's behaviour — the database layer is `src/db/incidents.ts` (was `src/db/acquiredConditions.ts`). `treatmentWindowGameDays`/`treatmentCostKey` moved out of the risk-model JSON onto real `incident_types` columns, since both are genuinely read elsewhere, unlike the columns this move found dead. The one real tie to genetics stays and is not a mistake: four of the twelve read a heritable robustness trait as an input to onset *risk*, which is never a test result and never a carrier status. The horse page's Incidents card now trims history to a display-only window (`incident_history_game_days`, default 720 = two game years) measured from `resolve_game_day` — rows are never deleted, since `/admin/incidents`'s outcome distribution is the only instrument that says whether slice 0020's untuned numbers are right; an open incident and any `degenerative` outcome ignore the window, and a hidden count is always stated when something is left out. **Part B** puts a one-word verdict (Poor / Weak / Acceptable / Good / Outstanding) next to each conformation trait once `horse_show_summary.starts >= 1`, computed by `src/engines/conformation/labels.ts` from the judge's own per-trait `traitScoreFor` (`src/engines/showing/score.ts`, factored out of `scoreEntry` for exactly this reuse — CLAUDE.md §13's no-second-scoring-path rule applied to a display) against the horse's own breed's `ideal_vector`, with band edges as four new live config keys. The bars are shortened into a `.meter-line` flex row (`min-width: 0` on `.meter`) so the word doesn't overflow off a phone screen. The breeding preview gets the labels too, words-only in a small table, one row per trait, mare and stallion each judged against **their own breed's** ideal (cross-breed pairings are possible) with a plain sentence that a foal is not the average of its parents. A horse with no starts, or whose breed has no `ideal_vector`, reads **Unknown** rather than rendering nothing. See `docs/slices/0022-incidents-untangled-and-conformation-labels.md`. |
...
- **Condition** vs **incident** (slice 0022) — a condition is inherited: a fact about a genotype, tested once, permanent, living in `conditions`/`horse_conditions`. An incident is an event: colic, laminitis and the ten others, novelty and chance rather than something a horse carries or a test reveals, living in its own `incident_types`/`horse_incidents`. A horse can be affected by a condition; a horse *has* an incident, and it resolves.
